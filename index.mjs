...
var encode_1 = lib.encode = encode;
...
const striptags = (string, secure = true) => (secure
    ...
    : String(string?.trim() || '')
        ...
        ?.trim() || '') || '';
const entities = {
    decode: (string, secure = true) => (secure ? striptags(decode_1(string?.trim() || ''), false) : decode_1(string?.trim() || '')) || '',
    encode: (string) => encode_1(string?.trim() || '', { mode: 'nonAsciiPrintable' }) || '',
};
...
export { cx, dates, entities, forceArray, head, isEmpty, notEmpty, s, sAll, sEl, sElAll, setTime, striptags, tokenGenerate, xss };
